 ...
 * 
 * *****************  Version 2  *****************
 * User: Morris       Date: 11/21/03   Time: 6:09p
 * Updated in $/mmlib/GridCtrl
 * Add property to control "AutoTab" action
 * 
 * *****************  Version 1  *****************
 ...
 * 
 * *****************  Version 2  *****************
 * User: Morris       Date: 10/13/03   Time: 12:12a
 * Updated in $/Judson/PhotoTest/gridctrl
 * Add cell grouping
 * 
 * *****************  Version 5  *****************
 * User: Morris       Date: 3/03/03    Time: 4:31p
 * Updated in $/Metorex/C100sxt/GridCtrl
 * Notify the owner when focus changes (like windows common control does)
 * 
 * *****************  Version 4  *****************
 * User: Morris       Date: 3/03/03    Time: 12:14p
 * Updated in $/Metorex/C100sxt/GridCtrl
 * Implement "Retain Selection" property like real list control does
 * 
 * *****************  Version 3  *****************
 * User: Morris       Date: 2/28/03    Time: 3:55p
 * Updated in $/Metorex/C100sxt/GridCtrl
 * Replace with more evolved version from Zylux Zip project, but add some
 * enhancements made in the C100 project
 * 
*/
/////////////////////////////////////////////////////////////////////////////
#include "stdafx.h"
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    {
    case VK_DELETE: 
        if (0 < m_idCurrentCell.row &&
            IsCellEditable(m_idCurrentCell.row, m_idCurrentCell.col))
        {
...
// MLM Implement auto-add row
    ...
    if (m_bAutoTab &&
        (VK_RETURN == pgvItem->lParam)) 
      pgvItem->lParam = VK_TAB;
  ...
  // MLM 3/3/2003 Prevent autostart of editing in new cell - more consistent operation
...
void CGridCtrl::OnDraw(CDC* pDC)
{
    ...
    for (row = 0; row < m_nFixedRows; row++)
    {
        ...
        for (col = minVisibleCol; col <= maxVisibleCol; col++)
        {
            ...
            if (pCell)
                pCell->Draw(pDC, row, col, rect, FALSE);
            eGroupType eGrp = pCell->GetGrouped();
            if (notGrp != eGrp)
            {
              strGrp += pCell->GetText();
              if (beginGrp == eGrp) // first grouped cell
              {
               rectGrp = rect;
               memcpy(&lf, pCell->GetFont(), sizeof(LOGFONT));
               bGrouped = true;
              }
              else if (endGrp == eGrp)
              {
                rectGrp.right = rect.right;
                CFont Font;
                lf.lfWeight = 600;
                Font.CreateFontIndirect(&lf);
                pDC->SelectObject(&Font);
                pDC->SetTextColor(GetFixedTextColor());
                pDC->SetBkColor(GetFixedBkColor());
                DrawText(pDC->m_hDC, strGrp, -1, rectGrp, DT_CENTER | DT_NOPREFIX);
                Font.DeleteObject();
                rectGrp.left = rectGrp.right+1;
                strGrp.Empty();
                bGrouped = false;
              }
            }
...
BOOL CGridCtrl::RedrawCell(int nRow, int nCol, CDC* pDC /* = NULL */)
{
    ...
    if (IsCellGrouped(CCellID(nRow, nCol)))
      return FALSE;
...
void CGridCtrl::UnSelectAllCells(bool bForceRepaint /* = false */)
{
  CDC* pDC = NULL;
  if (bForceRepaint)
      pDC = GetDC();
...
int CGridCtrl::InsertRow(int nHeading, int nRow /* = -1 */)
{
  CString strHeading;
  strHeading.Format(TEXT("%d"), nHeading);
  return InsertRow(strHeading, nRow);
}
...
BOOL CGridCtrl::SetItemData(int nRow, int nCol, UINT lParam)
{
    CGridCell* pCell = GetCell(nRow, nCol);
    if (!pCell)
        return FALSE;
...
UINT_PTR CGridCtrl::GetItemData(int nRow, int nCol) const
{
    CGridCell* pCell = GetCell(nRow, nCol);
    if (!pCell)
        return (LPARAM) 0;
...
void CGridCtrl::AutoSize()
{
    ...
    for (nCol = 0; nCol < nNumColumns; nCol++)
        for (nRow = 0; nRow < nNumRows; nRow++)
        {
            ...
            if (pCell)
              size = pCell->GetCellExtent(pDC);
            if (size.cx >(int) m_arColWidths[nCol] && pCell && notGrp == pCell->GetGrouped())
...
bool CGridCtrl::IsCellGrouped(CCellID &cell) const
{
  CGridCell *pCell = GetCell(cell.row, cell.col);
  return pCell->GetGrouped() != notGrp;
}
...
{
  if (!m_bRetainSelection)
    {
    CCellID idcurr = m_idCurrentCell;
    SetFocusCell(-1, -1);
    UnSelectAllCells();
    m_idCurrentCell = idcurr;
    }
  SendMessageToParent(m_idCurrentCell.row, m_idCurrentCell.col, NM_KILLFOCUS, (UINT)(UINT_PTR)pNewWnd);
}

void CGridCtrl::OnSetFocus(CWnd *pNewWnd)
{
  if (!m_bRetainSelection)
    {
  ...
  SendMessageToParent(m_idCurrentCell.row, m_idCurrentCell.col, NM_SETFOCUS, (UINT)(UINT_PTR)pNewWnd);
}
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
  ...
  // (If the user moves the mouse, then dragging occurs)
    ...
    if (nFlags & MK_CONTROL)
      ...
      if (!IsCellGrouped(m_LeftClickDownCell))
