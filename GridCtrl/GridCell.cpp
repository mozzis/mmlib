    ...
    BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    if((m_nState & GVIS_FOCUSED) || (m_nState & GVIS_DROPHILITED))
    {
        ...
        {
            if (notGrp == m_eGroup || endGrp == m_eGroup)
              rect.right--;
            rect.bottom--;
        }
        ...
        else if (notGrp == m_eGroup || endGrp == m_eGroup)
          rect.right--;
    ...
    if (bDrawFixed && m_pGrid->m_nGridLines != GVL_NONE)
    {
        ...
        // highlight it.
        if (bHiliteFixed && notGrp == m_eGroup)
        {
            lf.lfWeight = SELECTED_CELL_FONT_WEIGHT;
        ...
        else
        {
            ...
            if(notGrp == m_eGroup || beginGrp == m_eGroup)
...
void CGridCell::EndEdit()
{
  if (m_pEditWnd)
    ...
    else if (m_pEditWnd->IsKindOf(RUNTIME_CLASS(CInPlaceCWNumEdit)))
      ((CInPlaceCWNumEdit*)m_pEditWnd)->EndEdit();
    else
      ASSERT(FALSE);
    }
}
