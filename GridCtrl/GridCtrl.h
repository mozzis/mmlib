 ...
 * 
 * *****************  Version 2  *****************
 * User: Morris       Date: 11/17/03   Time: 3:31p
 * Updated in $/mmlib/GridCtrl
 * Add "AutoTab" property to control if Enter key automatically tabs to
 * next entry
 * 
 * *****************  Version 1  *****************
 ...
 * 
 * *****************  Version 4  *****************
 * User: Morris       Date: 10/18/03   Time: 6:21p
 * Updated in $/Judson/PhotoTest/gridctrl
 * Make Grid's OnDraw public so can call from view
 * 
 * *****************  Version 3  *****************
...
// 
// *****************  Version 8  *****************
// User: Maynard      Date: 11/14/00   Time: 11:03a
// Updated in $/ZyluxZip/Grid
// Add code for insert item notification
 * 
 * *****************  Version 7  *****************
 ...
 * 
 * *****************  Version 6  *****************
 * User: Maynard      Date: 3/15/00    Time: 1:58p
 * Updated in $/ZyluxZip/Grid
 * Make GetCell public so dialog parent can use its knowledge of cell
 * types
 * 
 * *****************  Version 5  *****************
 * User: Maynard      Date: 3/06/00    Time: 12:37p
 * Updated in $/ZyluxZip/Grid
 * Add constants for auto-insert row string and notification
 * 
 * *****************  Version 4  *****************
 * User: Maynard      Date: 3/02/00    Time: 8:40p
 * Updated in $/ZyluxZip/Grid
 * Flip-Flop on IMPLEMENT_DYNAMIC/DYNCREATE
 * 
 * *****************  Version 3  *****************
...
typedef struct _GV_ITEM {
    ...
    UINT_PTR lParam;      // 32-bit value to associate with item
...
#define GVN_INSERTITEM          LVN_INSERTITEM
...
#define GVN_AUTOINSERTROW       LVN_LAST+10

#define GRID_AUTOINSERT_STRING  _T("*")
...
public:
    ...
    inline CSize GetCellTextExtent(int nRow, int nCol)
        {   return GetTextExtent(nRow, nCol, NULL);     }
    ...
    void SetAutoTab(BOOL bAutoTab = TRUE)         { m_bAutoTab = bAutoTab;            }
    BOOL IsAutoTab() const                        { return m_bAutoTab;                }
    ...
    void SetRetainSelection(BOOL bRetain = TRUE)  { m_bRetainSelection = bRetain;     }
    BOOL GetRetainSelection() const               { return m_bRetainSelection;        }
    ...
    BOOL   SetItemData(int nRow, int nCol, UINT lParam);
    UINT_PTR GetItemData(int nRow, int nCol) const;
...
public:
    virtual int  InsertColumn(LPCTSTR strHeading, UINT nFormat = DT_CENTER|DT_VCENTER|DT_SINGLELINE,
                      int nColumn = -1);
    virtual int  InsertRow(int nHeader, int nRow = -1);
    virtual int  InsertRow(LPCTSTR strHeading, int nRow = -1, bool bResetRow = true);
    ...
    bool IsCellOneClickEdit(CCellID &cell) const;
    bool IsCellGrouped(CCellID &cell) const;
...
public:
    virtual ~CGridCtrl();
	virtual BOOL OnWndMsg(UINT msg, WPARAM wP, LPARAM lP, LRESULT *pR)
	{ return CWnd::OnWndMsg(msg, wP, lP, pR); }

    CGridCell* GetCell(int nRow, int nCol) const;
    static BOOL RegisterWindowClass();
protected:
    LRESULT SendMessageToParent(int nRow, int nCol, int nMessage, UINT nFlags = 0);
...
public:
    ...
    inline void SetShadedPrintOut(BOOL bEnable = TRUE)
        {   m_bShadedPrintOut = bEnable;    }
    inline BOOL GetShadedPrintOut(void)
...
protected:
    void UnSelectAllCells(bool bForceRepaint = false);
    void SelectAllCells();
