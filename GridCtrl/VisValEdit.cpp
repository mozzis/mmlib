...
CVisValidEdit::CVisValidEdit()
{
	...
	//default charset
	m_setCurrent = VVEset::SET_ALLCHAR;
	//create storage for the 'more info' strings - 1 for each set
// VVEset {SET_CUSTOM, SET_ALLCHAR, SET_ALPHANUM, SET_ALPHA, SET_DECIMAL, SET_HEXADECIMAL, SET_OCTAL, SET_BINARY, SET_FLOAT };
...
CBrush * CVisValidEdit::CustomValidation()
{
	...
	if(::SendMessageTimeout(pParent->GetSafeHwnd(),UWM_VVE_DO_CUSTOMVALIDATION,
				(WPARAM)&nm,
				(LPARAM)(LPCSTR)s,SMTO_BLOCK,1,&dwResult) == 0) //if == 0 then timeout or failure occured
	{
...
//returns VVE_LEN_SHORT, VVE_LEN_LONG or VVE_OK
int CVisValidEdit::CheckLen( LPCSTR str)
{
	if(m_bDoLengthCheck == FALSE || m_bNoValidation == TRUE)
		return VVE_OK;
...
void CVisValidEdit::SetValidChars(LPCTSTR cstrCustomChar)
{
	...
	for (UINT i = 0; i < strlen(cstrCustomChar); i++) 
...
void CVisValidEdit::SetValidCharSet(VVEset vveset, LPCTSTR cstrCustomChar)
{
	...
	case SET_CUSTOM: //user choice 
		if(cstrCustomChar == NULL)
			return ;
		for (i = 0; i < strlen(cstrCustomChar); i++) 
			m_bValidChar[(UCHAR)cstrCustomChar[i]] = TRUE;
		SetMoreInfoString(vveset);
		m_bEnableRangeCheck = FALSE;
		break;
	case SET_ALPHANUM:
		for ( i = 0; i < strlen(VVE_DEFSETALPHANUM); i++) 
		{
		...
		break;
	case SET_ALPHA:
		for ( i = 0; i < strlen(VVE_DEFSETALPHA); i++) 
		{
		...
		break;
	case SET_DECIMAL:
		for ( i = 0; i < strlen(VVE_DEFSETDECIMAL); i++) 
		{
		...
		break;	
	case SET_SDECIMAL:
		for ( i = 0; i < strlen(VVE_DEFSETSDECIMAL); i++) 
		{
		...
		break;
	case SET_OCTAL:
		for ( i = 0; i < strlen(VVE_DEFSETOCTAL); i++) 
		{
	...
	case SET_HEXADECIMAL:
		for ( i = 0; i < strlen(VVE_DEFSETHEXADECIMAL); i++) 
		{
		...
		break;
	case SET_BINARY:
		for ( i = 0; i < strlen(VVE_DEFSETBINARY); i++) 
		{
		...
		break;
	case SET_FLOAT:
		for ( i = 0; i < strlen(VVE_DEFSETFLOAT); i++) 
		{
...
BOOL CVisValidEdit::GetWindowValue(long &lVal)
{
	...
	case SET_DECIMAL:
		...
		lVal = (long)strtoul(s,&pEnd,10); 
		break;
	case SET_OCTAL:
		iRangeCheck = CheckRange(s);
		lVal = (long)strtoul(s,&pEnd,8);
		break;
	case SET_HEXADECIMAL:
		iRangeCheck = CheckRange(s);
		lVal = (long)strtoul(s,&pEnd,16);
		break;
	case SET_BINARY:
		iRangeCheck = CheckRange(s);
		lVal = (long)strtoul(s,&pEnd,2);
		break;
	case SET_FLOAT:
		{
...
{
	CString strVal = "";
	char szTemp[LONG_STRMAX] = "";//long enough for representation of a long as a string 
	switch(m_setCurrent) 
	{
	case SET_DECIMAL:
		strVal = ultoa(lSetVal,szTemp,10);
		SetWindowText(strVal);
		break;
	case SET_SDECIMAL:
		strVal = ltoa(lSetVal,szTemp,10);
		SetWindowText(strVal);
		break;
	case SET_OCTAL:
		strVal = ultoa(lSetVal,szTemp,8);
		SetWindowText(strVal);
		break;
	case SET_HEXADECIMAL:
		strVal = ultoa(lSetVal,szTemp,16);
		SetWindowText(strVal);
		break;
	case SET_BINARY:
		strVal = ultoa(lSetVal,szTemp,2);
		SetWindowText(strVal);
		break;
	case SET_FLOAT:
...
BOOL CVisValidEdit::SetWindowValue(long lSetVal)
{
	...
	case SET_SDECIMAL:
		...
		break;
	case SET_OCTAL:
		strVal = ltoa(lSetVal,szTemp,8);
		SetWindowText(strVal);
		break;
	case SET_HEXADECIMAL:
		strVal = ltoa(lSetVal,szTemp,16);
		SetWindowText(strVal);
		break;
	case SET_BINARY:
		strVal = ltoa(lSetVal,szTemp,2);
		SetWindowText(strVal);
		break;
	case SET_FLOAT:
		return SetWindowValue((double)lSetVal);
...
/*		if(dSetVal < 0)
		...
		break;
	case SET_BINARY:
		strVal = ultoa((unsigned long)dSetVal,szTemp,2);
/*		if(dSetVal < 0)
			bRangeOK = (LONG_MIN < (__int64)dSetVal && LONG_MAX > (__int64)dSetVal);
		else
			bRangeOK = (ULONG_MAX > (unsigned __int64)dSetVal);  */
...
//returns VVE_OK, VVE_NO_SMALL or VVE_NO_LARGE
int CVisValidEdit::CheckRange(VVEset vveset, LPCSTR lpszVal)
{
	...
	case SET_DECIMAL:
		{
			...
			if(s.SpanIncluding(VVE_DEFSETSDECIMAL).GetLength() > 11)//include all valid chars (& sign)
				return VVE_NO_LARGE;
			__int64 i64 =  _atoi64(lpszVal);//get the window value as a 64bit int
			if(i64 < 0)
				return VVE_NO_SMALL;
