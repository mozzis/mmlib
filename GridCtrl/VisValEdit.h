...
public:
	int  CheckRange();
	int  CheckRange(LPCSTR lpszVal)
	{	return CheckRange(m_setCurrent,lpszVal);};
	static int CheckRange(VVEset vveset, LPCSTR lpszVal);
	...
	void SetContextInfo(VVEbrush ErrBrush, LPCSTR sErrMesg){m_arrErrStrs.SetAt(ErrBrush,sErrMesg);};
...
protected:
	...
	int    CheckLen(LPCSTR str);//returns VVE_LEN_SHORT , VVE_LEN_LONG or VVE_LEN_OK
