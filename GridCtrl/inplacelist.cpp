...
// 
// *****************  Version 6  *****************
// User: Maynard      Date: 11/14/00   Time: 1:00p
// Updated in $/ZyluxZip/Grid
// Add DECLARE/IMPLEMENT_DYNAMIC macros so can do runtime type checking
// Add call to EndEdit to OnCloseUp, temp fix since this doesn't scroll
// correctly
 * 
 * *****************  Version 5  *****************
 ...
 * 
 * *****************  Version 4  *****************
 * User: Maynard      Date: 3/22/00    Time: 10:12p
 * Updated in $/ZyluxZip/Grid
 * Change WM_CLOSE to DestroyWindow
 * 
 * *****************  Version 3  *****************
 * User: Maynard      Date: 3/15/00    Time: 1:59p
 * Updated in $/ZyluxZip/Grid
 * Better arrow key behavoir for lists - up/down selects list element,
 * right/left exits cell
 * 
 * *****************  Version 2  *****************
...
void CInPlaceList::EndEdit()
{
    ...
    dispinfo.item.lParam  = m_nLastChar; 
...
// Runs when the combo box drop-down closes after a new
// selection is made - want to update other columns
/////////////////////////////////////////////////////////////////////////////
void CInPlaceList::OnCloseup() 
{
    ...
    dispinfo.hdr.idFrom   = GetDlgCtrlID();
    dispinfo.hdr.code     = GVN_ENDLABELEDIT;
    // parent dialog processes the code to do the update
    dispinfo.item.mask    = LVIF_TEXT|LVIF_PARAM;
    dispinfo.item.row     = m_nRow;
    dispinfo.item.col     = m_nCol;
    dispinfo.item.strText = str;
    ...
    if (IsWindow(pOwner->GetSafeHwnd()))
        pOwner->SendMessage(WM_NOTIFY, GetDlgCtrlID(), (LPARAM)&dispinfo);
    // MLM:: TEMP FIX since this control isn't scrolled by the grid as it should be
    EndEdit();
}
